...
```jinja2 title="qwen3_rerank_template.jinja" {5-6}
...
`<think>\n\n</think>` 是空思考标记——引导模型跳过推理直接输出判断。配合 vLLM 的 `classifier_from_token: ["no", "yes"]` 配置，让引擎直接输出 "yes" token 的 softmax 概率作为相关性分数，而非走完整的文本生成流程。
