...
- **路由**：`/[locale]/blog` -> `src/app/[locale]/blog/page.tsx`
...
| 文件路径 | 说明 |
|----------|------|
| `src/app/[locale]/blog/page.tsx` | 博客列表页主文件 |
...
- `next/image` — 图片优化
- `next-intl` — 国际化（`useLocale` 用于日期格式化）
...
2. **标签 URL slug**：标签名通过 `tagToSlug()` 转为 URL 安全的 slug（如 `Next.js` → `next-js`），避免 middleware matcher 将含点号的路径排除
...
8. **日期格式化**：使用 `useLocale()` 获取当前 locale，映射为 BCP-47 标签（`zh` → `"zh-CN"`，`en` → `"en-US"`），传入 `toLocaleDateString()` 实现国际化日期
