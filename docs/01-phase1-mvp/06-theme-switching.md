...
`next-themes` 默认使用 **localStorage** 存储主题偏好（非 Cookie）。`storageKey` 仅控制 localStorage 的键名，并不会启用 cookie 存储：
...
<ThemeProvider
  ...
  defaultTheme="system"
  enableSystem
  storageKey="finn-days-theme"  // 自定义 localStorage 键名
>
```

> **注意**: `next-themes` 通过在 `<head>` 中注入阻塞脚本来防止 FOUC（主题闪烁），不需要服务端读取 cookie。对于博客场景，localStorage 已完全足够。如果未来确实需要 SSR 可读主题状态，需要额外自定义 cookie 方案，但当前不推荐这样做。
