...
Finn Days 是一个面向中文技术社区的个人博客平台，用于发布技术实践分享、记录开发心得、展示个人项目。目标是打造一个**内容至上、体验优雅、长期可维护**的现代化技术博客。
...
| `next-intl` | 国际化 (i18n) | App Router 原生支持，类型安全的翻译键 |
...
│   │   ├── layout.tsx                  # 根布局: 字体加载、ThemeProvider、Navbar、Footer
│   │   ├── page.tsx                    # 首页: Hero + 最近文章 + 技能展示
│   │   ├── globals.css                 # 全局样式: CSS 变量体系 + Tailwind 导入
│   │   ├── not-found.tsx               # [Phase 1] 自定义 404 页面
│   │   ├── sitemap.ts                  # [Phase 3] 动态 Sitemap 生成
│   │   ├── robots.ts                   # [Phase 3] Robots.txt 配置
│   │   │
│   │   ├── feed.xml/                   # [Phase 1] RSS Feed
│   │   │   └── route.ts               # Route Handler: 生成 RSS 2.0 XML
│   │   │
│   │   ├── blog/                       # [Phase 1] 博客模块
...
| 路由 | 类型 | 渲染方式 | 所属阶段 | 说明 |
|------|------|---------|---------|------|
| `/` | 页面 | SSG | 基础框架 | 首页 (Hero + 最近文章 + 技能) |
| `/blog` | 页面 | SSG | Phase 1 | 博客列表 (分页 + 标签筛选) |
| `/blog/[slug]` | 动态页面 | SSG | Phase 1 | 文章详情 (MDX 渲染) |
| `/projects` | 页面 | SSG | Phase 1 | 项目展示 |
| `/about` | 页面 | SSG | Phase 1 | 关于我 |
| `/tags` | 页面 | SSG | Phase 1 | 标签索引 |
| `/tags/[tag]` | 动态页面 | SSG | Phase 1 | 按标签筛选文章 |
| `/notes` | 页面 | SSG | Phase 4 | 短笔记列表 |
| `/feed.xml` | Route Handler | 静态 | Phase 1 | RSS 2.0 Feed |
| `/sitemap.xml` | 元数据文件 | 静态 | Phase 3 | Sitemap |
...
```
src/app/
├── layout.tsx              # 根布局: <html> + <body> + ThemeProvider + Navbar + Footer
├── page.tsx                # 首页 (可能使用不同于博客的布局)
│
...
> **注意**: 路由分组 `(main)` 为可选方案。初期可不使用分组，待页面增多后按需引入以复用布局。
...
| # | 任务 | 预计工作量 | 前置依赖 | 产出 |
|---|------|-----------|---------|------|
| 1.1 | 集成 Velite 内容层 | 中 | 无 | `velite.config.ts`, `content.ts` |
| 1.2 | 定义 Post/Project Schema | 小 | 1.1 | Zod Schema + TypeScript 类型 |
| 1.3 | 创建内容查询 API | 中 | 1.2 | `src/lib/content.ts` |
| 1.4 | 抽取 Navbar 组件 | 小 | 无 | `components/layout/navbar.tsx` |
| 1.5 | 抽取 Footer 组件 | 小 | 无 | `components/layout/footer.tsx` |
| 1.6 | 重构 layout.tsx | 小 | 1.4, 1.5 | 统一布局 |
| 1.7 | 重构 page.tsx (首页) | 中 | 1.3, 1.6 | 从 Velite 读取真实数据 |
| 1.8 | 博客列表页 | 中 | 1.3 | `app/blog/page.tsx`, `PostCard` |
| 1.9 | 文章详情页 | 大 | 1.3 | `app/blog/[slug]/page.tsx` |
| 1.10 | Shiki 代码高亮集成 | 中 | 1.9 | rehype-pretty-code 配置 |
| 1.11 | TOC 目录导航 | 中 | 1.9 | `components/blog/toc.tsx` |
| 1.12 | 标签系统 | 中 | 1.3 | `app/tags/`, Badge 组件 |
| 1.13 | 暗色/亮色主题 | 中 | 1.6 | next-themes + ThemeToggle |
| 1.14 | About 页面 | 小 | 1.6 | `app/about/page.tsx` |
| 1.15 | Projects 页面 | 小 | 1.3, 1.6 | `app/projects/page.tsx` |
| 1.16 | RSS Feed | 小 | 1.3 | `app/feed.xml/route.ts` |
| 1.17 | 撰写首批文章 | 中 | 1.9 | 2-3 篇 MDX 文章 |

**里程碑**: 博客可上线，支持文章发布、代码高亮、标签分类、主题切换、RSS 订阅
...
| 4.4 | 短笔记系统 | 中 | Phase 1 | `content/notes/`, `app/notes/` |
| 4.5 | 国际化 (i18n) | 大 | Phase 1 | next-intl 配置 + 翻译文件 |
| 4.6 | View Transitions | 小 | Phase 1 | CSS + JS API 集成 |
...
```bash
# 生产依赖
npm install velite shiki rehype-pretty-code next-themes feed @tailwindcss/typography
...
```bash
npm install cmdk @codesandbox/sandpack-react next-intl
```
